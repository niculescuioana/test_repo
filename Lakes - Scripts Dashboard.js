...
var scriptSheet = 'https://docs.google.com/spreadsheets/d/1trwcjeUMXD4zAPPd_5ID1lv-hsCjKs8P5VdSC1mdX0s/edit#gid=1687970652';
...
var MASTER_EMAIL_LIST = ['ricky@pushgroup.co.uk','charlie@pushgroup.co.uk'];
...
function main() {
  var accounts = MccApp.accounts().withCondition('Name = "Lakes Cottage Holiday"').executeInParallel('runScript');
}
...
function runScript() {
  ...
  var GENERAL_INPUTS = {};
  var generalInputsTabData = masterSpreadsheet.getSheetByName('General Inputs').getDataRange().getValues();
  ...
  var ACCOUNT_INPUTS = {};
  var commonInputsTabData = masterSpreadsheet.getSheetByName('Common Inputs').getDataRange().getValues();
  ...
  var masterSheet = masterSpreadsheet.getSheetByName(configSheet);
  var configData = masterSheet.getDataRange().getValues();	
