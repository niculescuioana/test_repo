/*************************************************
* Master Reference Script - Set 2
* @author Naman Jindal <naman@pushgroup.co.uk>
* @version 1.0
***************************************************/

// Url of the Dashboard
var scriptSheet = 'https://docs.google.com/spreadsheets/d/1kZv_q2rDzk0UuPn4qj54hGPauzror1-W2WQkFeB7T-8/edit';
...
var masterConfigSheet = 'https://docs.google.com/spreadsheets/d/1KKyOuyxSN-FlpUt4xyRyevIHhWBEOlZopTpMFsPT_74/edit';
...
function runScript(input) {
  ...
  var accName = AdWordsApp.currentAccount().getName(),
      accId = AdWordsApp.currentAccount().getCustomerId();
...
function setupMonthlyTabs() {
  ...
  if(!GENERAL_INPUTS['DAILY_DASHBOARD_URL']) {
    return;
  }
  
  
  var dateString = Utilities.formatDate(new Date(), AdWordsApp.currentAccount().getTimeZone(), 'MMM dd, yyyy HH:mm:ss')
  var date = new Date(dateString);
  
  var daysPassed = round((date.getDate() - 1 + date.getHours()/24),2);
  date.setDate(1);
  ...
  var thisMonth = Utilities.formatDate(new Date(), 'UTC', 'MMM yyyy');
  
  var date = new Date(Utilities.formatDate(new Date(), 'UTC', 'MMM d, yyyy'));
  date.setDate(0);
  
  var lastMonth = Utilities.formatDate(date, 'PST', 'MMM yyyy');
  var ss = SpreadsheetApp.openByUrl(GENERAL_INPUTS['DAILY_DASHBOARD_URL']);
  var tmTab = ss.getSheetByName(thisMonth);
  var lmTab = ss.getSheetByName(lastMonth);
  
  if(!tmTab && !lmTab) {
    tmTab = SpreadsheetApp.openByUrl(MASTER_TEMPLATE).getSheetByName('Franchisee Daily Dashboard').copyTo(ss);
    tmTab.setName(thisMonth);
  }
  
  if(!tmTab) {
    ss.setActiveSheet(lmTab);
    tmTab = ss.duplicateActiveSheet();
    tmTab.setName(thisMonth);
  }
}