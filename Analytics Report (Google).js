...
var REPORT_TAB = 'Analytics Report (Google)';
...
function main() {
  ...
  var headerRow = [getAdWordsFormattedDate(2, 'MMM dd, yyyy'), getAdWordsFormattedDate(3, 'MMM dd, yyyy'), '30 Day Avg',
                   TM, LM, PM, TMLY, LMLY, PMLY, LM, PM, LM, PM];
...
function gatherStats(SETTINGS) {
  ...
  var TO_LM = Utilities.formatDate(now, 'PST', 'yyyy-MM-dd');
  
  now.setDate(1);
  var FROM_LM = Utilities.formatDate(now, 'PST', 'yyyy-MM-dd');
  
  now.setDate(0);
  var TO_PM = Utilities.formatDate(now, 'PST', 'yyyy-MM-dd');
  
  now.setDate(1);
  var FROM_PM = Utilities.formatDate(now, 'PST', 'yyyy-MM-dd');
  
  for(var k in ids) {
    getDataFromMCF(ids[k],stats,SETTINGS['LM'],FROM_LM,TO_LM);
    getDataFromMCF(ids[k],stats,SETTINGS['PM'],FROM_PM,TO_PM);    
  }
  
  outputRow.push(stats[SETTINGS['LM']].assistedConversions, stats[SETTINGS['PM']].assistedConversions, 
                 stats[SETTINGS['LM']].assistedConversionValue, stats[SETTINGS['PM']].assistedConversionValue);
...
function getLinkedProfileIds(UA_NUMBER) {
  ...
  /*if(UA_NUMBER.toString().indexOf('UA') < 0) {
...
function initStatsMap(SETTINGS,today) {
  ...
  map['Total'] = { transactions:0, revenue:0, assistedConversions: 0 }
  map[SETTINGS['TM']] = { transactions:0, revenue:0 }
  map[SETTINGS['LM']] = { transactions:0, revenue:0, assistedConversions: 0, assistedConversionValue: 0 }
  map[SETTINGS['PM']] = { transactions:0, revenue:0, assistedConversions: 0, assistedConversionValue: 0 }
...
function getCpcSourceGoalCompletions(id,FROM,TO) {
  var optArgs = { 'dimensions': 'ga:date', 'filters': 'ga:medium==cpc;ga:source==google' };
  var attempts = 3;
  var results;
...
function getDataFromMCF(id,stats,key,FROM,TO) {
  ...
  for(var k in rows) {
    ...
    stats[key].assistedConversions += parseInt(rows[k][2].primitiveValue,10);
    stats[key].assistedConversionValue += parseFloat(rows[k][3].primitiveValue);    
  }
}
