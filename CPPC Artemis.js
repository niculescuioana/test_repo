...
var CONFIG_URLS = {
    ...
    'Artemis': 'https://docs.google.com/spreadsheets/d/1kIieaWVt9QoQSvggZzggMMEtw8TZEW2lEy7-NMmmtZA/edit'
  };
  ...
  var VIEW_EMAILS = [
    'Kate.Watts@artemis.marketing', 'Mike.Knivett@artemis.marketing', 'mike@artemisinternetmarketing.com', 'lester.pratt@artemis.marketing'
  ];
  ...
  var LOGO_URL = 'https://artemis.marketing/wp-content/themes/prelethal_fp/img/artemis-logo.png';
  var LOGO_ROW_OFFSET = 3;
  ...
  function main() {
    ...
    accounts
    //.withCondition('Name = "Red Box Teachers"')
    .withLimit(50)
  ...
  function parseInputs() {
   var accounts = MccApp.accounts();
    ...
    accounts = accounts.withLimit(50).get();
    var map = {};
    while(accounts.hasNext()) {
      var acc = accounts.next();
      map[acc.getCustomerId()] = { 'NAME': acc.getName(), 'ROW_NUM': 0 }; 
    }
    ...
    for(var k in commonInputsTabData) {
      if(!map[commonInputsTabData[k][1]]) { 
        Logger.log(commonInputsTabData[k][1]);
        continue;
      }
      
      map[commonInputsTabData[k][1]]['ROW_NUM'] = parseInt(k,10)+2;
      
      for(var j in commonInputsTabData[k]) {
        map[commonInputsTabData[k][1]][commonInputsHeader[j]] = commonInputsTabData[k][j];
      }  
    }
    
    for(var account in map) {
      if(!map[account]['ROW_NUM']) {
        map[account]['ROW_NUM'] = commonInputsTab.getLastRow()+1;
        commonInputsTab.getRange(map[account]['ROW_NUM'],1,1,2).setValues([[map[account]['NAME'], account]]);
      }
    }
  }
