...
var scriptSheet = 'https://docs.google.com/spreadsheets/d/1bfmBZ6MhaHJaICKWwobkPc4RpP6TgH9m9YD-cv9M1oU/edit';

// Label for the dashboard
var LABEL = 'CG Managed';

var MASTER_EMAIL_LIST = [
  'roy@clickguardian.co.uk','adwords@clickguardian.co.uk','clickfraud@clickbuffer.com'
];

var SUB_MCC_ID = '470-648-3435';
...
function updateManagerLabels() {
  ...
  var ss = SpreadsheetApp.openByUrl(scriptSheet);
  var commonInputsTabData = ss.getSheetByName('Account Inputs').getDataRange().getValues();
  commonInputsTabData.shift();
  
  var accountMap = {};
  var commonInputsHeader = commonInputsTabData.shift();
  for(var k in commonInputsTabData) {
    accountMap[commonInputsTabData[k][0]] = 1;
  }
 
  var iter = MccApp.accounts()
  .withCondition("ManagerCustomerId IN ['" + SUB_MCC_ID + "']")
  .get();
  while(iter.hasNext()) {
    ...
    while(labelIter.hasNext()) {
      var labelName = labelIter.next().getName();
      if(labelName == LABEL) {
        if(!accountMap[accName]) {
          account.removeLabel(LABEL); 
        } else {
          delete accountMap[accName];
        }
      }
    }
